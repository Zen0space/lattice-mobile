...
export const useWidgetManager = (
    ...
    async (dashboardId: string) => {
      try {
        ...
        if (config.enableDevMode) {
          if (__DEV__) {
            console.log(
              `✅ Loaded ${dashboardWidgets.length} widgets for dashboard: ${dashboardId}`
            );
          }
        }
      } catch (error) {
  ...
  const addWidget = useCallback(
    async (dashboardId: string, widgetData: Partial<Widget>) => {
      try {
        setIsLoading(true);
        setError(null);
        ...
        if (
          ...
          (config.maxWidgetsPerDashboard || DEFAULT_CONFIG.maxWidgetsPerDashboard!)
        ) {
  ...
  const updateWidget = useCallback(
    async (dashboardId: string, widgetId: string, updates: Partial<Widget>) => {
      try {
        setIsLoading(true);
        setError(null);
    ...
    async (dashboardId: string, widgetId: string) => {
      try {
        ...
        if (config.enableDevMode) {
          if (__DEV__) {
            console.log(
              `✅ Deleted widget "${widgetToDelete.title}" from dashboard: ${dashboardId}`
            );
          }
        }
      } catch (error) {
        ...
        if (config.enableDevMode) {
          console.error(
            `❌ Error deleting widget ${widgetId} from dashboard ${dashboardId}:`,
            error
          );
        }
  ...
  const reorderWidgets = useCallback(
    async (dashboardId: string, widgetIds: string[]) => {
      try {
        setIsLoading(true);
        setError(null);
        ...
        // Reorder widgets
        const reorderedWidgets = widgetIds.map(id => currentWidgets.find(w => w.id === id)!);
            ...
            { text: 'Cancel', style: 'cancel' },
            {
              ...
              onPress: async () => {
                try {
                  ...
                  const errorMessage =
                    error instanceof Error ? error.message : 'Failed to clear widgets';
                  setError(errorMessage);

                  if (config.enableDevMode) {
                    console.error(
                      `❌ Error clearing widgets from dashboard ${dashboardId}:`,
                      error
                    );
                  }
    ...
    async (dashboardId: string, widgetId: string) => {
      try {
        ...
        if (config.enableDevMode) {
          if (__DEV__) {
            console.log(
              `✅ Duplicated widget "${widgetToDuplicate.title}" in dashboard: ${dashboardId}`
            );
          }
        }
      } catch (error) {
        ...
        if (config.enableDevMode) {
          console.error(
            `❌ Error duplicating widget ${widgetId} in dashboard ${dashboardId}:`,
            error
          );
        }
   ...
   */
  const getWidgetCount = useCallback(
