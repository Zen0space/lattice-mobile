...
const ChatBubble: React.FC<ChatBubbleProps> = ({
            ...
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={{ color: '#1f2937', fontWeight: '600', fontSize: 16 }}>
                ...
                {typeof financialData.currentPrice === 'number'
                  ? financialData.currentPrice.toLocaleString()
                  : financialData.currentPrice}
              </Text>
              ...
              <View style={{ marginRight: 16, marginBottom: 4 }}>
                ...
                <Text
                  style={{
                    ...
                    fontWeight: '500',
                    color:
                      financialData.technicalAnalysis.trend === 'bullish'
                        ? '#10b981'
                        : financialData.technicalAnalysis.trend === 'bearish'
                          ? '#ef4444'
                          : '#f59e0b',
                  }}
                >
        ...
        {financialData.recommendation?.action && (
          <View
            style={{
              ...
              borderRadius: 12,
              backgroundColor:
                financialData.recommendation.action === 'BUY' ? '#dcfce7' : '#fee2e2',
            }}
          >
            <Text
              style={{
  ...
  const renderMultipleCharts = () => {
    ...
    if (financialData.specificAllocations) {
      return (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 12 }}>
          {financialData.specificAllocations.slice(0, 3).map(
    ...
    if (financialData.specificAllocations && Array.isArray(financialData.specificAllocations)) {
      const allocationsWithCharts = financialData.specificAllocations.filter(
        (allocation: any) => allocation.chartConfig?.showChart
      );
  ...
  return (
    <View className={`mb-4 ${isUser ? 'items-end' : 'items-start'}`}>
      <View
        className={`flex-row items-start ${isUser ? 'flex-row-reverse' : ''}`}
        style={{ maxWidth: '90%' }}
      >
        {/* Avatar */}
        <View
          className="w-8 h-8 rounded-full items-center justify-center mt-1 mx-2"
          style={{
            backgroundColor: isUser ? '#6B7280' : '#10a37f',
            flexShrink: 0,
          }}
        >
          <Text className="text-white text-xs font-semibold">{isUser ? 'You' : 'AI'}</Text>
        </View>
        ...
        >
          {/* Message Text */}
          <Text
            className={`text-base leading-5 ${isUser ? 'text-white' : 'text-gray-800'}`}
            style={{ lineHeight: 20 }}
          >
            {displayMessage}
          </Text>
          ...
          {/* Timestamp */}
          <Text className={`text-xs mt-3 ${isUser ? 'text-blue-100' : 'text-gray-500'}`}>
